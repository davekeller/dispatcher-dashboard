...
The Stops section begins with a compact, full-width blue-slate navigation band: a near-black icon tile and a value-first `12/20 delivered` title on the left, route actions and List/Map mode on the right. `Stops` remains as the small uppercase label above the live count. The band uses a lighter translucent slate fill and only horizontal keylines—no radius, side border, or shadow—so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll.
