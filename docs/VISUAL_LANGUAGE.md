...
The Stops section begins with a viewport-wide, single-row white navigation band. Its surface breaks out of the route content's 20px gutters to touch the route rail on the left and Lookout on the right while the title and controls retain the normal content alignment. A near-black icon tile and value-first `12/20 delivered` title lead the row; Remaining, Schedule, and HOS fit occupy an evenly spaced three-column grid in the center; and List/Map mode anchors the right edge. Soft standard vertical dividers separate these zones without nested cards, tinted metric panels, or decorative top rules. The mode control is flat and shadowless, using the shared pale-slate selected fill against a quiet canvas-colored track. `Stops` remains the small uppercase line above the live count; completion percentage stays in the route rail. The delivered count remains the largest reading at 22px; secondary route readings step down to 20px and compact further when the expanded route rail narrows the workspace. Semantic meaning stays concentrated in each status value and its dot. Time and schedule language uses forms such as `1h8m late` and `43m spare`, keeping the complete operational status visible without wrapping the band. These readings belong here rather than in the expanded route rail: the bar keeps them visible in either list or map mode, while the rail stays focused on stop navigation. The band has square edges, simple horizontal keylines, and no outer shadow, so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
