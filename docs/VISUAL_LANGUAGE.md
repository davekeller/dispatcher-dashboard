...
The Stops section begins with a full-width blue-slate navigation band: a near-black icon tile and a value-first `12/20 delivered` title on the left, with List/Map mode on the right. `Stops` and the completion percentage share the small uppercase line above the live count. A dedicated full-width divider row beneath carries Remaining, Schedule, and HOS fit. Primary readings use the display face at 22–24px against those 8px labels, deliberately exceeding the driver card's 20px operating metrics so live route progress leads the hierarchy without clipping longer status values. These four route-status readings belong here rather than in the route rail: the bar keeps them visible in either list or map mode, while the rail stays focused on stop navigation. The band uses a lighter translucent slate fill and only horizontal keylines—no radius, side border, or shadow—so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
