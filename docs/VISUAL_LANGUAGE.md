...
The Stops section begins with a viewport-wide, two-level blue-slate navigation band: its surface breaks out of the route content's 20px gutters to touch the route rail on the left and Lookout on the right, while the title and controls retain the normal content alignment. A near-black 32px icon tile and value-first `12/20 delivered` title occupy the lighter top level, with List/Map mode on the right. The mode control is flat and shadowless, using the shared pale-slate selected fill. `Stops` remains the small uppercase line above the live count; completion percentage stays in the route rail. A dedicated edge-to-edge divider row beneath carries Remaining, Schedule, and HOS fit over a quieter translucent surface. Each metric gets a restrained 3px top rule—slate for Remaining and its semantic status hue for Schedule and HOS—plus simple vertical dividers. Primary readings use the display face at 22–24px against those 8px labels, deliberately exceeding the driver card's 20px operating metrics so live route progress leads the hierarchy without clipping longer status values. These readings belong here rather than in the expanded route rail: the bar keeps them visible in either list or map mode, while the rail stays focused on stop navigation. The band has square edges, horizontal slate keylines, and no outer shadow, so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
