...
The Stops section begins with a compact cool blue-slate instrument bar: a near-black icon tile and delivered count on the left, route actions and List/Map mode on the right. Its restrained slate keyline and slightly stronger shadow distinguish persistent navigation from the white receipt cards below. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll.
