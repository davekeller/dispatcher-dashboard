...
Route cards use the same distinct title row as Lookout recommendations. A compact overall-route status dot, matched to the board-lane header dots, leads the route id; the highest-priority badge—Over limit, Approaching limit, or Behind schedule—and countdown share the right edge. Directly below the dot, the stop spine continues down the left of the driver assignment and progress row, turning the overall status marker into the visual head of route progress. Driver name and truck plate sit inline beside the avatar, with muted refresh age at the row's upper right:
