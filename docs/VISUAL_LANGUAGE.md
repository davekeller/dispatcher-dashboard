...
The top-level driver header keeps two layers inside one card: its padded identity and live-countdown section above a full-width five-column grid for Driving today, On duty, Break, Stops, and Driving left vs. limit. The grid touches the card edges, uses one horizontal boundary plus simple vertical dividers, and gives every cell the same padding and vertical centering—no nested metric cards or gutters. Active alerts move into a separate card directly below. Every alert is one horizontal row with an icon in a narrow first column, status and explanation in the flexible middle, and a no-wrap action group aligned to the right; multiple alerts share the card and divide into rows. Critical alert cards receive a restrained red keyline around the full card. An over-limit driver also adds a 3px Act now red top border across the two-layer driver card, reinforcing the badge and countdown without tinting the surface.

The Stops section begins with a compact, full-width blue-slate navigation band: a near-black icon tile and a value-first `12/20 delivered` title on the left, with List/Map mode on the right. `Stops` remains as the small uppercase label above the live count. The band uses a lighter translucent slate fill and only horizontal keylines—no radius, side border, or shadow—so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
