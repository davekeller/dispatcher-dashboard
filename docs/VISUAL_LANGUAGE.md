...
The product bar adds one industrial counterweight to those lighter surfaces: an original navigation-slate line mark forms a `D` whose spine receives three inbound route origins. It stands alone without a tile, preserving filled backgrounds for selected navigation states only. A compact 16px uppercase Dispatch wordmark in the display face follows on white, with firm weight and open tracking so the lockup reads as a logistics identity rather than a stock icon beside interface text. The bar itself and every control embedded in it are shadowless. Board and Map form one 176px edge-to-edge segmented control centered horizontally in the main product bar; equal-width halves give the primary workspace switch more visual prominence without competing with the brand. The outer keyline defines the control, a single divider separates the views, and the selected slate fill occupies its entire segment without an inset card, ring, gap, or shadow. A driver/route detail is a child of Board, so Board remains selected while its route breadcrumb stays with the left brand group. Lookout's Chat and Artifacts tabs reuse the same flat selected fill instead of a separate underline language. Artifacts is intentionally an empty-state workspace until durable Lookout outputs are defined.
