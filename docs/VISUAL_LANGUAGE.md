...
The Stops section begins with a compact, full-width blue-slate navigation band: a near-black icon tile and a value-first `12/20 delivered` title on the left, followed by a divider grid for Remaining, Schedule, and HOS fit, with List/Map mode on the right. `Stops` and the completion percentage share the small uppercase line above the live count. Primary readings use a larger 14–15px scale against those 8px labels so the live route state leads the band. These four route-status readings belong here rather than in the route rail: the bar keeps them visible in either list or map mode, while the rail stays focused on stop navigation. With the rail collapsed the readings share one compact line; opening the rail moves the three secondary cells to a full-width line beneath the title and controls so their values remain legible. The band uses a lighter translucent slate fill and only horizontal keylines—no radius, side border, or shadow—so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
