...
The product bar adds one industrial counterweight to those lighter surfaces: the truck logo is an icon-only warm near-black `ink` tile, followed by the Dispatch wordmark as dark text on the white bar. The bar itself and every control embedded in it are shadowless. Board and Map form one edge-to-edge segmented control: the outer keyline defines the control, a single divider separates the views, and the selected slate fill occupies its entire segment without an inset card, ring, gap, or shadow. A driver/route detail is a child of Board, so Board remains selected while its route breadcrumb is visible. Lookout's Chat and Artifacts tabs reuse the same flat selected fill instead of a separate underline language. Artifacts is intentionally an empty-state workspace until durable Lookout outputs are defined.
...
The top-level driver header keeps two layers inside one card: its padded identity and live-countdown section above a full-width five-column grid for Driving today, On duty, Break, Stops, and Driving left vs. limit. The grid touches the card edges, uses one horizontal boundary plus simple vertical dividers, and gives every cell the same padding and vertical centering—no nested metric cards or gutters. A 3px top rule always anchors this card: deep red when over limit, brighter red for other Act now routes, amber for Watch, blue for On break, medium slate for Offline, and the palette's darker navigation slate for Clear. The sticky Stops/status band follows the driver card, before exception detail. Active alerts then use a separate card below the band. Every alert is one horizontal row with an icon in a narrow first column, status and explanation in the flexible middle, and a no-wrap action group aligned to the right; multiple alerts share the card and divide into rows. Critical alert cards receive a restrained red keyline around the full card.

The Stops section begins with a viewport-wide, single-row blue-slate navigation band: its medium-light slate gradient is deliberately darker than the route field, and its surface breaks out of the route content's 20px gutters to touch the route rail on the left and Lookout on the right while the title and controls retain the normal content alignment. A near-black icon tile and value-first `12/20 delivered` title lead the row; Remaining, Schedule, and HOS fit occupy an equal three-column grid in the center; and List/Map mode anchors the right edge. Vertical dividers keep these zones distinct without nested cards. The mode control is flat and shadowless, using the shared pale-slate selected fill. `Stops` remains the small uppercase line above the live count; completion percentage stays in the route rail. Each metric gets a restrained 3px top rule—slate for Remaining and its semantic status hue for Schedule and HOS. The delivered count remains the largest reading at 22px; secondary route readings step down to 20px and compact further when the expanded route rail narrows the workspace. Time and schedule language uses forms such as `1h8m late` and `43m spare`, keeping the complete operational status visible without wrapping the band. These readings belong here rather than in the expanded route rail: the bar keeps them visible in either list or map mode, while the rail stays focused on stop navigation. The band has square edges, stronger horizontal slate keylines, and no outer shadow, so it reads as section structure rather than another card. It remains sticky at the top of the route workspace beneath the product bar, keeping selection and mode controls available while the receipts scroll. At rest it shows no batch actions. Selecting one or more unresolved receipts reveals only `Reassign selected (n)` beside List/Map; schedule-reset and customer-notification actions stay in their route-alert and Lookout contexts.
