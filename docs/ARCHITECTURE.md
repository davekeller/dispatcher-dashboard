...
The product logo is an original line-built `D`: three origin nodes and inbound route lines feed its vertical spine while the outer stroke forms the destination-facing letter. It stands alone in navigation slate with no background tile, so it cannot be mistaken for the filled selected states beside it. The 29px mark remains centered in a 64px cell, the collapsed route rail's width, directly over the route file's back arrow. A 16px uppercase Bricolage wordmark follows as separate dark text on the white bar, using deliberate open tracking to read as identity rather than a navigation label. The bar's right inset matches the mark's left inset so the avatar sits symmetrically; the product bar casts one subtle 16px slate wash downward when it is the lowest layer of navigation; on a route file the Stops bar is the lowest layer and casts the same wash instead, so the shadow always sits under the last row of nav, and Lookout casts it leftward. Board and Map share a 176px segmented shell centered on the main product bar, independent of the brand, breadcrumb, clock, and avatar widths. Each half receives equal width; the outer keyline and single internal divider are the same slate ink as the selected fill, so the selected segment's edge is crisp. Each link reaches the shell edges, and the selected view fills its full segment with `nav-selected-ink` under white type—no inner padding, radius, ring, or shadow. Route files remain within the Board workspace, so Board keeps that selected state while the route breadcrumb stays with the left brand group. The same shared state helper gives Chat and Artifacts the flat fill treatment without introducing another shadow language.
