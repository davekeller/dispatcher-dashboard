...
2. **Stops instrument.** A sticky Stops navigation band follows the driver card and sits full-width above both exception detail and stop content, keeping the four route-status readings and List/Map mode available while scrolling. The value-first `12/20 delivered` title carries Progress and its percentage; a divided horizontal grid beside it carries Remaining with ping age, Schedule, and HOS fit. It uses a lighter translucent blue-slate surface, horizontal slate keylines, and a near-black icon tile. It has no radius, side keyline, or shadow, so it reads as structural navigation rather than another receipt card. No batch action occupies the resting bar; selecting at least one unresolved receipt reveals only `Reassign selected (n)` beside the mode control. Schedule-reset and notification actions remain available from the alert and Lookout flows where their reason is visible.
