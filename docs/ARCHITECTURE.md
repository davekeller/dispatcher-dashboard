...
**Route card.** A shared title row leads with an overall-route status dot at the same compact scale as the board-lane header dots and the route id. Route-level operating exceptions such as Won't finish and Offline follow the id as plain semantic text, then a compact muted ping age; status remains fully visible while the lower-priority age may truncate in a narrow lane. The highest-priority Over limit, Approaching limit, or Behind schedule badge pairs with the live HOS countdown on the right, exactly as it does in Lookout recommendations. The full age wording remains in the title tooltip; Lookout omits this optional board metadata. The miniature route spine begins directly beneath that dot and occupies a narrow full-height strip beside the card rows, so the title indicator becomes the visual head of the route's progress. The first content row uses a four-unit grid: driver identity spans two units, Stops one, and Up next one. Its 50% divider aligns exactly with the HOS fit / Route risk divider below, while the progress cells share the same padding and right alignment. Driver name stacks over the truck license identifier beside the avatar, matching the value-over-label rhythm of `14 / 16 Stops` and `#15 Up next`. Every stop remains a node in one evenly spaced route scale. Successfully delivered stops use smaller 5px connective nodes; unresolved, failed, late, and post-HOS work uses a fixed 6px node. Nothing compresses or magnifies by progress percentage. Completed work grades from light sage to dark clear-green, viable undelivered work is gray, and failed, past-due, or post-HOS work is red. The route-file timeline keeps one literal labeled row per stop, with only delivered rows tightened. Secondary alert reasons render as plain text in the footer unless promoted to title metadata, and Lookout's first-pick marker follows only when present. Clear cards stay quiet but keep their full height and readable identity in narrow lanes. Click opens the route file. Nothing drags: a card's position is computed, not assigned.
...
4. **Stop receipts.** Oldest first, each the scroll target of its rail node. Every receipt uses a vertically centered case-file row with four horizontal columns: a narrow first column gives the semantic marker and bare stop number stable internal slots, using the same sans-serif semibold number weight as the rail at a larger but restrained size; the second holds customer, address, status, instructions, and the neutral dispatcher note; the third carries the small horizontal event track; and the remaining right side is a consistently padded, single-row grid of four facts. The shared marker renders delivered as a graded green check, viable undelivered as gray outline, late or post-HOS as red outline, failed as solid red, and unassigned as dashed. Completed stops read Arrived → Left → Signed and surface On-site, Outcome, Load, and Priority. Pending stops read Planned → Projected → Window and surface Projected ETA, Window, Drive in, and Load. Fact values are slightly larger than event metadata. A vertical ellipsis floats at the right edge without its own cell or divider and opens the existing stop actions. The next stop is highlighted; stops past the limit carry a chip; notified stops show the stamp; unassigned stops show "needs a driver."
