...
| 45 | The dev panel became the simulated-shift control: the clock is the button, the panel anchors under it, says what the simulation is, and explains each demo action | Reviewers didn't find a panel at the bottom; the committee needs to know at a glance that this is a simulated day they can scrub |
| 46 | Recommendation cards in the Lookout bar are compact: avatar beside the directive, countdown on the right, no header row | The title already carries the name; one row less per card is the whole point of the bar |
