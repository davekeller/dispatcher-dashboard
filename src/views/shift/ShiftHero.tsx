...
export default function ShiftHero({ metrics, ranked, filters, onPreset }: { metrics: Metrics; ranked: DriverCard[]; filters: FilterState; onPreset: (filters: FilterState) => void }) {
  ...
  const items: Metric[] = [
    ...
    { id: 'offline', label: 'Offline', value: count('offline'), preset: { ...EMPTY_FILTERS, band: ['offline'] }, dot: 'bg-offline-fill' },
    { id: 'break', label: 'On break', value: count('break'), preset: { ...EMPTY_FILTERS, band: ['break'] }, dot: 'bg-break-fill' },
    { id: 'clear', label: 'Clear', value: count('clear'), preset: { ...EMPTY_FILTERS, band: ['clear'] }, dot: 'bg-clear-fill' },
  ]
  ...
  return (
    <section className="iq-metrics-band shrink-0 border-b border-line px-5 py-4 text-ink" aria-label="Shift status">
