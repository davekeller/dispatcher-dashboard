 ...
 * recommendations; History is the shift's record, read from the store's event log. */
export default function LookoutSidebar() {
  ...
  return (
    ...
    <div className={`lookout-slot min-w-0 shrink-0 ${collapsed ? 'is-closed w-0' : 'w-[20rem] 2xl:w-[26rem]'}`} aria-hidden={collapsed}>
    <aside className="lookout-panel flex h-full w-[20rem] shrink-0 flex-col 2xl:w-[26rem] border-l border-line" aria-label={`${LOOKOUT.name}, the shift co-pilot`} inert={collapsed || undefined}>
