...
export default function RecommendationsBar({ open, onToggle, summary, count, children }: { open: boolean; onToggle: () => void; summary: string; count: number; children: ReactNode }) {
  return (
    <section aria-label={`${LOOKOUT.name} recommends`}>
      <button type="button" onClick={onToggle} aria-expanded={open} className="sticky top-0 z-10 flex w-full items-center gap-2 border-b border-lookout/20 bg-lookout-soft px-3 py-2 text-left">
        ...
        {count > 0 && <span className="tnum rounded-full bg-act-now px-1.5 text-[10px] font-semibold leading-4 text-on-accent">{count}</span>}
      ...
      {open && <div className="flex flex-col gap-3 border-b border-line bg-well/40 p-3">{children}</div>}
    </section>
  )
}
