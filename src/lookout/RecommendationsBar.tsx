...
export default function RecommendationsBar({ open, onToggle, summary, children }: { open: boolean; onToggle: () => void; summary: string; children: ReactNode }) {
  return (
    <section aria-label={`${LOOKOUT.name} recommends`}>
      <button type="button" onClick={onToggle} aria-expanded={open} className="lookout-soft-surface sticky top-0 z-10 flex w-full items-center gap-2 border-b border-lookout/20 px-3 py-2 text-left font-lookout">
