  ...
  return (
    ...
    >
      <div className="lookout-composer-shell flex h-11 items-center gap-2 rounded-full border-2 py-1 pl-2 pr-1">
        ...
        <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={`Ask ${LOOKOUT.name}…`} aria-label={`Ask ${LOOKOUT.name}`} className="min-w-0 flex-1 bg-transparent font-lookout text-[13px] text-ink outline-none placeholder:text-muted" />
