...
/** Renders whatever ACTIVITY_FILTERS holds: text filters as a search box, multi filters as a chip row with All. */
export default function ActivityFilters({ filters, onChange, shown, total }: { filters: FilterState; onChange: (next: FilterState) => void; shown: number; total: number }) {
  ...
  return (
    <div className="sticky top-0 z-10 flex shrink-0 flex-col gap-2 border-b border-line bg-panel/90 px-3 py-2 backdrop-blur">
      {ACTIVITY_FILTERS.filter((f) => f.kind === 'text').map((f) => (
      ...
      {ACTIVITY_FILTERS.filter((f) => f.kind === 'multi').map((f) => {
