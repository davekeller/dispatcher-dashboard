...
const SIZE = { xs: 'text-[12px]', sm: 'text-[13px]', md: 'text-lg', lg: 'text-5xl leading-none tracking-tight' } as const

/** The number that matters. The same monospace as the route ids, tabular so it never jitters; tilde when it is an estimate. */
export default function Countdown({ minutes, stale, size = 'sm', className = '' }: { minutes: number; stale: boolean; size?: keyof typeof SIZE; className?: string }) {
  ...
  return (
    <span className={`tnum font-mono font-semibold ${SIZE[size]} ${color} ${className}`} title={stale ? 'Estimate: last ping is stale' : undefined}>
