...
import { historyFeed } from './derive'
import { HISTORY_FILTERS, EMPTY_HISTORY_FILTERS, applyHistoryFilters, isFilteringHistory } from './filters'
...
const feed = historyFeed(events, d)

describe('history filters', () => {
  it('is two filters: what she did, and a search', () => {
    expect(HISTORY_FILTERS.map((f) => f.id)).toEqual(['family', 'search'])
  })
  it('keeps everything when nothing is chosen', () => {
    expect(applyHistoryFilters(feed, EMPTY_HISTORY_FILTERS)).toHaveLength(feed.length)
    expect(isFilteringHistory(EMPTY_HISTORY_FILTERS)).toBe(false)
  })
  it('narrows to a family', () => {
    const only = applyHistoryFilters(feed, { ...EMPTY_HISTORY_FILTERS, family: ['contact'] })
    expect(only).toHaveLength(1)
    expect(only[0].kind === 'card' && only[0].event.seq).toBe(2)
    expect(isFilteringHistory({ ...EMPTY_HISTORY_FILTERS, family: ['contact'] })).toBe(true)
  })
  it('searches names and customers, case-insensitively', () => {
    expect(applyHistoryFilters(feed, { ...EMPTY_HISTORY_FILTERS, search: 'harbor' }).map((i) => i.kind === 'card' && i.event.seq)).toEqual([3])
    expect(applyHistoryFilters(feed, { ...EMPTY_HISTORY_FILTERS, search: 'DRE' }).every((i) => i.driverId === 'drv-03')).toBe(true)
  })
})
