# Dispatch — the Active Shift dashboard
...
**Live:** https://dispatch.kidastro.com · **Source:** this repo
...
npm test         # derivation tests: HOS math, rules, ranking, bands, seed, actions, plans, history
npm run build
```
...
4. **Lookout**, the rail on the right, reads the same ranked list the board does. If they ever disagree, that is the bug. Open a route and it turns into a plan for that driver: which stops to hand to whom and with how much room, where to put the reset, which customers to call, each button opening its dialog already filled in. Ask it "What should I do about Marcus?" and it answers with the same plans. The ✦ chip on the board's top card explains why that card is first and how the order works. The History tab is the shift's record: every decision as a card, with what was true when she made it.
5. **Map.** The Map tab shows the whole fleet: quiet trucks as small dots in their band color, the ones that need attention with a name-and-countdown pill, dark trucks hollow at their last known fix. Pick one and its whole route draws, with Lookout focused on that driver. On any route file, the List | Map toggle puts that route on a map, the truck a live fix gliding along its leg with the clock. Every position is derived from the receipts and the last ping, never stored (`src/geo/truckPosition.ts`).
6. **`src/alerts/rules.ts`.** Every alert is one object. Adding a rule is appending one.
7. **Lena's page.** Her avatar at the end of the product bar opens a settings view for the dispatcher and, beside it, the story of the build in six chapters: project, persona, problems, solutions, why, and progress, with the design decks' own slides where they belong.

## How it is built

[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) is the source of truth. In one line: a deterministic seeded fleet plus one clock → pure derivation (`hos/compute.ts`, `alerts/rules.ts`, `alerts/rank.ts`, `bands.ts`) → one ranked array → every surface. Actions are pure transforms with a confirm step and one-level undo. Every choice and cut is one line in [`docs/DECISIONS.md`](docs/DECISIONS.md).
...
Built: the board with its three lenses, the route file with receipts, a timeline rail and a map, the action dialogs with preview and undo, and the Lookout rail with recommendations, plans, chat, and history. Not built: the driver's phone view, real routing, auth, dark mode, mobile layouts. Chat is a handful of matched intents, not a model.
